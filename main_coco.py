...
def run_alphapose(new):
    ...
    for image_name in new:
        ...
        command = ("./scripts/inference.sh configs/coco/resnet/256x192_res152_lr1e-3_1x-duc.yaml pretrained_models/fast_421_res152_256x192.pth %s ./result_dacon_test_coco/%s"%(image_name,image_n))
...
def json_process():
    ...
    for file_name in glob("./result_dacon_test_coco/*"):
    ...
    #print(tmp_1)
    with open("output_test_coco.json", "w") as f:  
...
def json_to_csv():
    with open("output_test_coco.json", "r") as alpha:
        data = json.load(alpha)
    #data_ = np.array(data)
    #print(data)
...
def csv_write(data):
    ...
    sample.to_csv('baseline_submission_coco.csv', index=False)
