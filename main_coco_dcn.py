...
def run_alphapose(new):
    ...
    for image_name in new:
        ...
        command = ("./scripts/inference.sh configs/coco/resnet/256x192_res50_lr1e-3_2x-dcn.yaml pretrained_models/fast_dcn_res50_256x192.pth %s ./result_dacon_test_coco_dcn/%s"%(image_name,image_n))
...
def json_process():
    ...
    for file_name in glob("./result_dacon_test_coco_dcn/*"):
    ...
    #print(tmp_1)
    with open("output_test_coco_dcn.json", "w") as f:  
...
def json_to_csv():
    with open("output_test_coco_dcn.json", "r") as alpha:
        data = json.load(alpha)
    #data_ = np.array(data)
    #print(data)
...
def csv_write(data):
    ...
    sample.to_csv('baseline_submission_coco_dcn.csv', index=False)
